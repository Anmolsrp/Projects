...
int main()
{
    ...
    while(1)
    {
        ...
        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);     // tokenization   // count has the return value of sscanf

            if(count == 1)
            {
                if(strcmp(command[0],"ls") == 0)       // return value of strcmp is 0 if the two strings are equal
                {
                    is_file();
                }
